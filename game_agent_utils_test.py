...
class SymmetryTest(unittest.TestCase):
    def test_reflections_and_rotations(self):
        ...
        for _ in range(250):
            ...
            self.assertEqual(sym.rotate_180(loc, w, h), sym.rotate_90(sym.rotate_90(loc, w, h), w, h))
            self.assertEqual(sym.rotate_270(loc, w, h), sym.rotate_180(sym.rotate_90(loc, w, h), w, h))
            self.assertEqual(sym.rotate_270(loc, w, h), sym.rotate_90(sym.rotate_180(loc, w, h), w, h))
            self.assertEqual(sym.rotate_270(loc, w, h), sym.rotate_90(sym.rotate_90(sym.rotate_90(loc, w, h), w, h), w, h))
            self.assertEqual(sym.rotate_180(loc, w, h), sym.reflect_vertical(sym.reflect_horizontal(loc, w, h), w, h))
